/**
 * Country and territory calling codes used by the quote form.
 *
 * The ISO code keeps options with a shared calling code (for example the
 * United States and Canada) distinct while the dial code is what is sent with
 * the phone number.
 */
...
/** @type {Array<{ code: string, name: string, dialCode: string }>} */
...
const COUNTRIES_BY_CODE = new Map(COUNTRY_CALLING_CODES.map((country) => [country.code, country]));

/** @param {string} countryCode */
export function getCountry(countryCode) {
  return COUNTRIES_BY_CODE.get(countryCode) ?? COUNTRIES_BY_CODE.get(DEFAULT_COUNTRY);
}
...
export function countryForDialCode(dialCode) {
  return COUNTRY_CALLING_CODES.find((country) => country.dialCode === dialCode)?.code ?? DEFAULT_COUNTRY;
}

/**
 * Splits a stored international number so the UI can show the calling code in
 * the select and only the national number in the adjacent input.
 *
 * @param {unknown} value
 * @param {string} preferredDialCode
 */
export function splitPhoneNumber(value, preferredDialCode = DEFAULT_COUNTRY_CODE) {
  const phone = String(value ?? '').trim();
  const match = phone.startsWith('+')
    ? COUNTRY_CALLING_CODES
        .filter((country) => phone.startsWith(country.dialCode))
        .sort((a, b) => b.dialCode.length - a.dialCode.length)[0]
    : null;
  const dialCode = match?.dialCode ?? preferredDialCode;
  const countryCode = match?.code ?? countryForDialCode(dialCode);
  const nationalNumber = phone.startsWith(dialCode)
    ? phone.slice(dialCode.length).replace(/^[\s().-]+/, '').trim()
    : phone;

  return { countryCode, dialCode, nationalNumber };
}

/** @param {string} dialCode @param {unknown} value */
export function formatInternationalPhone(dialCode, value) {
  const phone = String(value ?? '').trim();
  if (!phone) return '';
